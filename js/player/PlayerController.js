...
/*
 * BUG-001: هنا كانت ثوابت الفصول الأربعة (DEFAULT_SUN_OFFSET /
 * SUN_TINT_BY_SEASON / AMBIENT_BY_SEASON / FOG_DENSITY_BY_SEASON)
 * عالقة بعد فصل MF-12 — غير مُصدَّرة وغير مُستخدَمة في هذا الملف،
 * بينما App.js يستعملها في `applyDayNight()`. نُقلت إلى مصدرها
 * الصحيح (js/core/App.js). استيرادها من هنا مستحيل: App.js يستورد
 * PlayerController، فأي استيراد معاكس = دورة (R6).
 */
 ...
 *
 * تبقى في هذه الوحدة عن قصد: هي مالكة تركيب اللاعب (`root.position`)
 * في السطر أدناه، ولا يقرؤها App.js إطلاقًا — نقلها هناك كان سيصنع
 * ثابتًا ميتًا + دورة استيراد.
 */
